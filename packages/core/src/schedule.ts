...
type ScheduleEntry<Context> =
    | { runnable: Runnable<Context>; tag?: never }
    | { runnable?: never; tag: Tag<Context> };

export class Schedule<Context = unknown> {
    #dag = new DirectedGraph<Runnable<Context>>();
    readonly #runnableDefinitions = new Map<
        Runnable<Context>,
        RunnableDefinition<Context>
    >();
    readonly #tagDefinitions = new Map<
        ScheduleId,
    ...
    add(
        runnables: Runnable<Context>[],
        options?: AddManyOptions<Context>
    ): this;
    ...
    async run(context: Context): Promise<this> {
        if (this.#dirty) {
            this.build();
        }

        for (const runnable of this.#dag.sorted) {
    ...
    build(): this {
        ...
        for (const {
            runnable,
            options,
        } of this.#runnableDefinitions.values()) {
        ...
        for (const { id, options } of this.#tagDefinitions.values()) {
            ...
            const before = this.#resolveDependencies(
                options?.before,
                dag,
                tags
            );
            const after = this.#resolveDependencies(
                options?.after,
                dag,
                tags
            );
    ...
    #assertIdAvailable(id: ScheduleId): void {
        if (this.#runnables.has(id) || this.#tagDefinitions.has(id)) {
            throw new Error(
                `ID ${String(id)} already exists in the schedule`
            );
        }
    }
    ...
    #copyOrderingOptions(
        options?: OrderingOptions<Context>
    ): OrderingOptions<Context> | undefined {
        if (!options) {
            return undefined;
        }

        return {
            before: Array.isArray(options.before)
                ? [...options.before]
                : options.before,
